...
const content = {
  ...
  content:
    "<p><strong>Rust </strong>is an iron oxide, a <s>usually reddish brown oxide formed</s> by the reaction of iron and oxygen in the presence of water or air moisture. Several forms of rust are distinguishable both visually and by spectroscopy, and form under different <em>circumstances</em>.[1] Rust consists of hydrated iron(III) oxides Fe2O3·nH2O and iron(III) oxide-hydroxide (FeO(OH), Fe(OH)3).</p>",
...
const profile = {
  ...
  email: "austen@maccas.edu.au",
  logo:
    "https://res.cloudinary.com/dlh0pcycr/image/upload/v1602642387/CPortfolio/pnqjoyx5lvoab2gt7djf.png",
  resume:
    "https://res.cloudinary.com/dlh0pcycr/image/upload/v1602642336/CPortfolio/fe7xyf8oyxnpogj4bs9t.pdf",
};
...
test("Register User API call is correct", async () => {
  ...
  return expect(axios.post).toHaveBeenLastCalledWith(
    apiPaths.REGISTER_USER,
    user
  );
});
...
test("Edit Content API call is correct", async () => {
  ...
  return expect(axios.put).toHaveBeenLastCalledWith(
    `${apiPaths.CONTENT}${content.id}`,
    content
  );
});
...
test("Delete Content API call is correct", async () => {
  ...
  return expect(axios.delete).toHaveBeenLastCalledWith(
    `${apiPaths.CONTENT}${content.id}`
  );
});
...
test("Get Profile API call is correct", async () => {
  ...
  return expect(axios.get).toHaveBeenLastCalledWith(
    `${apiPaths.PROFILE}${profile.id}`
  );
});
...
test("Upload Resume API call is correct", async () => {
  ...
  return expect(axios.post).toHaveBeenLastCalledWith(
    `${apiPaths.ADD_RESUME}${profile.id}`,
    profile.resume
  );
});
